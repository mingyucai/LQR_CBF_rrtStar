...
class LQRrrtStar:
    ...
    def planning(self):
        ...
        self.path, path_continuous, control_path_list = self.extract_path(
            self.vertex[index]
        )
    ...
    def cal_LQR_new_cost(self, node_start, node_goal, cbf_check=True):
        ...
        px, py, traj_cost, u_sequence_cost = self.sample_path(
            wx, wy, u_sequence
        )  # TO DO: update LQR cost
        if wx is None:
            ...
            for vg in elite_Vg_leaves:
                ...
                while tStep < len(traj2vg[:, 1]):
                    ...
                    elite_cddtSample = [
                        pi_q_tStep,
                        vgcost2come,
                    ]  # This tuple contains the actual sample pi_q_tStep
    ...
    def extract_path(self, node_end):
        ...
        while node.parent is not None:
            ...
            path_continuous.extend(
                [node.state_parent_to_current[0], node.state_parent_to_current[1]]
            )
            node = node.parent
        path.append([node.x, node.y])

        u_path_list = [
            [u_path[i].item(0, 0), u_path[i].item(1, 0)] for i in range(len(u_path))
        ]
        u_path_list.reverse()
    ...
    def save_state_and_control_trajectory_as_numpy(self, state_list, control_list):
        cwd = os.getcwd()
        os_path_for_state = os.path.join(
            cwd, "output_state_control_trajs", "state_double_integrator_traj.npy"
        )
        os_path_for_control = os.path.join(
            cwd, "output_state_control_trajs", "control_double_integrator_traj.npy"
        )
