...
class MPC_controller:
    ...
    def calculate_local_reference(
        self, x_ref, t_step, x_current, target_index=0, tracking_mode="running_time"
    ):
    ...
    def simulation(self, x_ref):
        ...
        while time < self.simulation_max_time:
            x_local_ref, target_idx = self.calculate_local_reference(
                x_ref,
                t_step,
                x_current,
...
if __name__ == "__main__":
    ...
    interpolated_dist = (
        0.1  # [m] distance between interpolated position state, This is critical
    )
    ...
    path_to_continuous_waypoints = (
        ...
        / ".."
        / ".."
        / "experiment/output_state_control_trajs/state_double_integrator_traj.npy"
    ).resolve()
    ...
    MPC = MPC_controller(
        ...
        control_weight=np.diag([0.02, 0.02]),
        interpolated_dist=interpolated_dist,
        x_init=x_init,
        obstacles=obstacles,
        show_animation=True,
        save_animation=False,
